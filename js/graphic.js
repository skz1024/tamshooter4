...
export class graphicSystem {
  ...
  static digitalFontDisplay (inputText, x = 0, y = 0, wordWidth = 20, wordHeight = 30) {
    ...
    if (this.checkTransform()) {
      let output = this.canvasTransform(x, y, wordWidth, wordHeight)
      x = output.outputX
      y = output.outputY
  ...
  static imageDisplay (image, sliceX, sliceY, sliceWidth, sliceHeight, x, y, width, height, ...option) {
    ...
    if (arguments.length === 3) {
      if (this.checkTransform()) {
        let output = this.canvasTransform(arguments[1], arguments[2], image.width, image.height)
      ...
      if (this.checkTransform()) {
        let output = this.canvasTransform(arguments[1], arguments[2], arguments[3], arguments[4])
        this.#context.drawImage(image, Math.floor(output.x), Math.floor(output.y), Math.floor(output.width), Math.floor(output.height))
        this.restoreTransform()
      } else {
      ...
      if (this.checkTransform()) {
        let output = this.canvasTransform(x, y, width, height)
  ...
  static fillRect (x, y, width, height, color = null, alpha = null) {
    ...
    // 알파값이 있는 경우
    let prevAlpha = this.#context.globalAlpha
    if (alpha != null) {
  ...
  static clearCanvas () {
    if (this.checkTransform()) {
      let output = this.canvasTransform(0, 0, this.CANVAS_WIDTH, this.CANVAS_HEIGHT)
      this.#context.clearRect(output.x, output.y, output.width, output.height)
      this.restoreTransform()
    } else {
  ...
  static fillText (text = '', x, y, color = 'black', maxWidth = null) {
    ...
    if (this.checkTransform()) {
      let textWidth = this.#context.measureText(text)
      let output = this.canvasTransform(x, y, textWidth, parseInt(this.#context.font))
      if (maxWidth != null) {
